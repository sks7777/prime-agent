...
export class DaemonAgentConnection implements AgentConnection {
	...
	private handleTransportClose(error: Error): void {
		...
		if (closeReason === "shutdown") {
			...
			// A client that owns daemon recovery (ACP) rides out a supervisor
			// shutdown like an update restart: it relaunches the replacement
			// daemon itself and restores (or re-creates) the resident session.
	...
	private async restoreConnectionAfterDaemonRestart(options: DaemonRestartRestoreOptions): Promise<boolean> {
		...
		let lastError: unknown;
		// Adopt a restored session onto this connection: bump the revision,
		// re-attach, and resync once, yielding to a concurrent update-restart
		// recovery that took over meanwhile.
		const adoptSession = async (activeSessionId: string): Promise<boolean> => {
			if (this.disposed || this.terminalCloseEmitted) {
				return false;
			}
			this.sessionRevision++;
		...
		while (!this.disposed && !this.terminalCloseEmitted && Date.now() < deadline) {
			...
			try {
				...
				if (restored?.activeSessionId) {
					if (await adoptSession(restored.activeSessionId)) return true;
				} else if (recreateFallback) {
					// The replacement daemon tombstoned the old resident session: create
					// points the fresh supervisor at the persisted session file, which
					// resumes the transcript on a new worker. Retries join the bound.
					...
					if (!createResponse.success) {
						throw deserializeDaemonError(createResponse);
					}
					if (!isDaemonSessionSummary(createResponse.data)) {
						throw new Error("Daemon returned an invalid create response during session restore");
					}
					const created = createResponse.data;
					if (await adoptSession(created.activeSessionId ?? created.id)) return true;
				}
			} catch (error) {
				lastError = error;
			}
