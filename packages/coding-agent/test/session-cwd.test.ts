...
describe("session cwd handling", () => {
	...
	it("throws a controlled error before runtime creation when the stored cwd is missing", async () => {
		...
		writeSessionFile(sessionFile, join(fallbackCwd, "does-not-exist"));
	...
	// Readonly startup must never repair, migrate, or follow-and-rewrite the transcript on disk.
	it.each([2, 3])(
		...
		async (version) => {
			...
			// An explicit --cwd overrides the stored cwd instead of reporting it missing.
