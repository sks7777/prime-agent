...
function setup(options: { activeTools?: string[]; selectChoice?: string; editorText?: string } = {}) {
	let activeTools = options.activeTools ?? ["ipython", "bash", "edit"];
...
describe("plan-mode example extension", () => {
	it("preserves custom active tools while toggling plan mode", async () => {
		const { activeTools, runCommand, setActiveTools } = setup({
			activeTools: ["ipython", "bash", "edit", "echo_tool"],
		});

		await runCommand("plan");

		expect(activeTools()).toEqual(["ipython", "bash", "echo_tool", "questionnaire"]);
		expect(setActiveTools).toHaveBeenLastCalledWith(["ipython", "bash", "echo_tool", "questionnaire"]);

		await runCommand("plan");

		expect(activeTools()).toEqual(["ipython", "bash", "edit", "echo_tool"]);
		expect(setActiveTools).toHaveBeenLastCalledWith(["ipython", "bash", "edit", "echo_tool"]);
	});
